...
df['Predicted_Sales4'] = predictions
...
df['Predicted_Sales4'] = df['Predicted_Sales4'].astype(float)
df['Predicted_Sales4'] = df['Predicted_Sales4'].round(2)
# Converting Predicted_Sales4 to float and rounding to two decimal places
...
df_to_insert = df[['TransactionID', 'Predicted_Sales4']]

# # SQL to delete the column if it exists
delete_column_sql = """
ALTER TABLE ONR_FactTransactions
DROP COLUMN Predicted_Sales4;
"""

# SQL to add the column if it doesn't exist
add_column_sql = """
ALTER TABLE ONR_FactTransactions
ADD COLUMN Predicted_Sales4 FLOAT;
"""

# SQL to update the comment for the new column
update_comment_sql = """
ALTER TABLE ONR_FactTransactions
MODIFY COLUMN Predicted_Sales4 FLOAT COMMENT 'Predicted sales (UnitPrice * Quantity) with third machine learning model using XGBoost and Bayesian Optimization.';
"""
...
try:
    # Your SQL statement
    update_sql = """
    INSERT INTO ONR_FactTransactions (TransactionID, Predicted_Sales4)
    VALUES (%s, %s)
    ON DUPLICATE KEY UPDATE Predicted_Sales4 = VALUES(Predicted_Sales4);
    """

    # Convert DataFrame to a list of tuples for executemany
    data_to_insert = list(zip(df_to_insert['TransactionID'], df_to_insert['Predicted_Sales4']))
